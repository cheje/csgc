...
export default function StackedBars() {
  ...
  useEffect(() => {
    d3.csv(
      'https://gist.githubusercontent.com/cheje/03a298076db3822e04faaf9b21aadf80/raw/68676b3d01476af66d2bd461136978b655c610b6/tcie-bars.csv'
    ).then(data => {
      ...
      // color scale
      const colorScale = d3.scaleOrdinal().domain(boros).range(['#355070', '#6d597a', '#b56576', '#e56b6f', '#eaac8b'])

      // generate bar stacks
      const stackedBarsGen = d3.stack().keys(boros)
      ...
      // x axis
      const xAxis = d3.axisBottom(xScale).ticks(years.length).tickSizeOuter(0)
      ...
      // TITLE
      svg.append('text').attr('y', '2em').text('TCIE Filings by Borough').attr('class', 'chart--title')
